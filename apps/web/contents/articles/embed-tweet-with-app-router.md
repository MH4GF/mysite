...
ツイートのウェブページから[ツイートの埋め込み]を選択し、埋め込み用の HTML をコピーします。
...
対象となったこのサイトの概要について説明します。

- Next.js App Router で作られているブログサイト
- 記事はマークダウンファイルとしてリポジトリ内に保存されている
- マークダウンファイルを [contentlayer](https://contentlayer.dev/) で読み込み、サーバーコンポーネント上で [Unified.js](https://unifiedjs.com/) を利用し ReactElement に変換する
...
usePathname, useEffect を利用し、ページ遷移のタイミングで `twttr.widgets.load()` を呼び出します。`twttr.widgets.load()` は型定義がないため、ここで追加しています。  
